...
int main( int argc, char* argv[] )
{
  ...
  try 
  {
    ...
    BOOST_FOREACH(Subdomain::map_gid2lid_t::const_reference gidlid, s.homes() )
    {
      ...
      // dfdx = 6x + 3
      diff_t d_dx( FD_family_c2::instance(), g.stencil[i], XDIM, d.dx() );
      real dfdx = d_dx( fcn.data() );
      if ( std::abs( dfdx - 6*xyz[XDIM] - THREE ) > EPS )
      {
        std::cout << "Error: Unexpected value for dfdx: "; 
        std::cout.precision( 16 );
        std::cout << dfdx << ".  Expected: ";
        std::cout << 6*xyz[XDIM] + THREE << std::endl;
        return 1;
      }

      // dfdy = 8y + 4
      diff_t d_dy( FD_family_c2::instance(), g.stencil[i], YDIM, d.dx() );
      real dfdy = d_dy( fcn.data() );
      if ( std::abs( dfdy - 8*xyz[YDIM] - FOUR ) > EPS )
      {
        std::cout << "Error: Unexpected value for dfdy: " 
          << dfdy << ".  Expected: ";
        std::cout << 8*xyz[YDIM] + FOUR << std::endl;
        return 1;
      }

      // dfdz = 10z + 5
      diff_t d_dz( FD_family_c2::instance(), g.stencil[i], ZDIM, d.dx() );
      real dfdz = d_dz( fcn.data() );
      if ( std::abs( dfdz - 10*xyz[ZDIM] - FIVE ) > EPS )
      {
        std::cout << "Error: Unexpected value for dfdz: " 
          << dfdz << ".  Expected: ";
        std::cout << 10*xyz[ZDIM] + FIVE << std::endl;
        return 1;
      }
